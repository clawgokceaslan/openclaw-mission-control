...
function normalizeFields(value: unknown): AgentOutputFormatField[] {
  ...
  return value.map((raw) => {
    ...
    const valueType = typeof item.valueType === 'string' && ['string', 'number', 'boolean', 'array'].includes(item.valueType) ? item.valueType as AgentOutputFormatField['valueType'] : 'string'
    return {
  ...
  }).filter((item) => item.key.trim() || item.description.trim() || item.defaultValue?.trim() || item.children?.length)
}
...
export class OutputFormatRepository extends BaseRepository<OutputFormat> {
  ...
  async create(orgId: string, input: { name: string; description?: string; fields: AgentOutputFormatField[] }): Promise<OutputFormat> {
    const now = Date.now()
    ...
    await this.db.prepare(`INSERT INTO output_formats (id, organization_id, name, description, fields_json, created_at, updated_at)
      VALUES (@id, @organizationId, @name, @description, @fieldsJson, @createdAt, @updatedAt)`).run({
      id: item.id,
  ...
  async update(orgId: string, id: string, input: { name: string; description?: string; fields: AgentOutputFormatField[] }): Promise<OutputFormat | undefined> {
    const current = await this.get(id)
    if (!current || current.organizationId !== orgId) return undefined
    const now = Date.now()
    await this.db.prepare(`UPDATE output_formats
      SET name = @name, description = @description, fields_json = @fieldsJson, updated_at = @updatedAt
