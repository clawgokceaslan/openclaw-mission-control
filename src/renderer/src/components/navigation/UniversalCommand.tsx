...
export function UniversalCommand({ embedded = false, onClose, onOpenTaskCreate }: UniversalCommandProps) {
  ...
  const allRows = useMemo<SearchRow[]>(() => [
    ...
    ...agents.map((agent) => ({ id: `agent:${agent.id}`, title: agent.name, subtitle: `Agent · ${agent.title || agent.description || 'Configured'}`, type: 'Agent' as const, path: `${APP_ROUTES.AGENTS}?edit=${encodeURIComponent(agent.id)}`, state: { openEditId: agent.id, agent } })),
