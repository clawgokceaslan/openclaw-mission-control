      ...
      <div className={styles.detailPane}>
        ...
        <section className={styles.drawerSection}>
          ...
          {detailTab === 'subtasks' ? (
            ...
            <><div className={styles.detailSectionHeader}><div><h4>Agent</h4><p>{selectedTaskAgent ? `${selectedTaskAgent.name}${selectedTaskAgentIsDefault ? ` (${scope.selectedTaskAgentDefaultLabel ?? 'Default'})` : ''}` : 'Unassigned'}</p></div></div><AgentAssignmentPanel agent={selectedTaskAgent} agents={scope.agents ?? []} ctaDescription="Choose the main executor; its active Tools become catalog-only capability context." onChange={scope.setTaskAgent} inheritedLabel={selectedTaskAgentIsDefault ? scope.selectedTaskAgentDefaultLabel ?? 'Default' : undefined} canClear={!selectedTaskAgentIsDefault} /></>
          ) : detailTab === 'skills' ? (
            <><div className={styles.detailSectionHeader}><div><h4>Skills</h4><p>{(scope.selectedTaskSkills ?? task.skills ?? []).length} selected{scope.selectedTaskSkillsAreDefault ? ' · Project default' : ''}</p></div></div><SkillsAssignmentPanel selectedSkills={scope.selectedTaskSkills ?? task.skills ?? []} skills={scope.skills ?? []} source={scope.selectedTaskSkillsAreDefault ? 'Project default' : 'Task'} ctaDescription="Select procedural guidance for this task." onChange={scope.setTaskSkills} inheritedLabel={scope.selectedTaskSkillsAreDefault ? 'Project default' : undefined} canClear={!scope.selectedTaskSkillsAreDefault && (task.skills ?? []).length > 0} /></>
    ...
    <div className={styles.modalBody} ref={scope.modalBodyRef} style={scope.splitTemplate ? { gridTemplateColumns: scope.splitTemplate } as CSSProperties : undefined}>
      <div className={styles.detailPane}>
        ...
        <section className={styles.drawerSection}>
          ...
          {detailTab === 'agent' ? <><div className={styles.detailSectionHeader}><div><h4>Agent</h4><p>{scope.selectedSubtaskAgent?.name ?? 'Unassigned'}</p></div></div><AgentAssignmentPanel agent={scope.selectedSubtaskAgent} agents={scope.agents ?? []} ctaDescription="Choose the main executor; its active Tools become catalog-only capability context." onChange={scope.setSubtaskAgent} /></> : null}
          {detailTab === 'skills' ? <><div className={styles.detailSectionHeader}><div><h4>Skills</h4><p>{(scope.selectedSubtaskSkillOptions ?? []).length} selected</p></div></div><SkillsAssignmentPanel selectedSkills={scope.selectedSubtaskSkills ?? []} skills={scope.skills ?? []} source="Subtask" ctaDescription="Select procedural guidance for this subtask." onChange={scope.setSubtaskSkills} /></> : null}
