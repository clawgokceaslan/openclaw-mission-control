...
- Chat send and attach: the renderer uses one submit path for button and Enter, keeps `/plan` command state separate from the prompt body, forwards command metadata, and sends attachment metadata/bytes while the sent-message renderer shows square preview tiles for outgoing attachments.
